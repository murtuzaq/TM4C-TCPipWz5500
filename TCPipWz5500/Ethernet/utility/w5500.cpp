...
//extern "C"  void IINCHIP_Delay_ms(uint16_t); // one way
...
uint8_t W5500Class::write(uint16_t _addr, uint8_t _cb, uint8_t _data)
{
    ...
    SPI.transfer(_data);
#endif
    IINCHIP_SpiSendData(_addr >> 8);
    IINCHIP_SpiSendData(_addr & 0xFF);
    IINCHIP_SpiSendData(_cb);
    IINCHIP_SpiSendData(_data);
...
uint8_t W5500Class::read(uint16_t _addr, uint8_t _cb)
{
    ...
    IINCHIP_SpiSendData(_addr & 0xFF);
    IINCHIP_SpiSendData(_cb);
    uint8_t _data = IINCHIP_SpiSendData(0);
    resetSS();
    return _data;
}
